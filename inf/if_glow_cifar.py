...
from inf.train.losses import NegativeGaussianLoss_test
...
def create_model(emerging=False, snf=False, ff=False,inv_conv=False, inv_flow=True, inv_conv_no_pad=False,
    ...
    for l in range(num_blocks):
        ...
        if split_prior and l < num_blocks - 1:
            layers.append(SplitPrior(current_size, NegativeGaussianLoss_test))
            current_size = (current_size[0] // 2, current_size[1], current_size[2])

    return FlowSequential(NegativeGaussianLoss_test(size=current_size), 
                         *layers)
...
def main():
    ...
    config = {
        'name': f'3L-28K_IF_timings_C_{run_name}',
        'eval_epochs': 1,
        'sample_epochs': 1,
        'log_interval': 100,
        'lr': 1e-4,
        ...
        # model options
        'activation': 'SLR', # Spline, SLR, BSpline
        ...
        'inv_flow': True, # Inv_FlowUnit
        'if_kernel_size': 3, # 3 for Inv_FlowUnit
        'inv_conv': False, # Inv_Flow with padding
        'inv_conv_no_pad': False, # Inv_Flow without padding
        ...
        'test': False,
        'image_size': (2, 16, 16), # 16, 32, 64, 128, 256, 512, 1024
    ...
    if config['test']:
        ...
        # DataLoader setup for CIFAR10
        config['image_size'] = (3, 32, 32)
