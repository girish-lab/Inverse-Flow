...
from inf.layers.flowlayer import FlowLayer, mark_expensive, \
    ModifiedGradFlowLayer
from inf.utils.toeplitz import get_sparse_toeplitz, get_toeplitz_idxs
# from inf.utils.convbackward import conv2d_backward
...
class inv_conv_(autograd.Function):
    ...
    def forward(ctx, x, W): # inverse pass for the Inv-Flow
        ...
        output_x  = x * 0.0
        # print('inv_conv_, fwd', f'x.shape: {x.shape},', f'W.shape: {W.shape}', f'output_x.shape: {output_x.shape}')
        # print('inv_conv_, fwd', f'x.dtype: {x.dtype},', f'W.dtype: {W.dtype}', f'output_x.dtype: {output_x.dtype}')
        # print('inv_conv_, fwd', f'x.device: {x.device},', f'W.device: {W.device}', f'output_x.device: {output_x.device}')    
...
class inv_flow_with_pad(FlowLayer):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
    ...
    def reset_parameters(self):
        ...
        # print('inv_flow, reset_parameters', w_init) # w_init is the weight matrix, no k_nn = 1
        self.weight_fwd = nn.Parameter(w_init)
        
        self.mask = self.get_mask()
        for c_out in range(self.weight_fwd.data.shape[0]):
            # if c_out == self.weight_fwd.data.shape[0] - 1:
            self.weight_fwd.data[c_out, -1, -1, -1] = 1.0
            # self.weight_fwd.data[c_out, c_out+1:, -1, -1] = 0.0

        if self.order == 'TR':
            self.weight_fwd.data = torch.flip(self.weight_fwd.data, [3])
        
        elif self.order == 'BL':
            self.weight_fwd.data = torch.flip(self.weight_fwd.data, [2])
        
        elif self.order == 'BR':
            self.weight_fwd.data = torch.flip(self.weight_fwd.data, [2, 3])
    ...
    def forward(self, input, context=None, compute_expensive=False):
        ...
        if compute_expensive:
            ...
            if self.order == 'TR':
                ...
                self.output = torch.flip(self.output, [3])

            elif self.order == 'BL':
                input = torch.flip(input, [2])
                self.weight_fwd.data = torch.flip(self.weight_fwd.data, [2])
                self.output = inv_conv_4d(input, self.weight_fwd)
                self.output = torch.flip(self.output, [2])

            elif self.order == 'BR':
                input = torch.flip(input, [2, 3])
                self.weight_fwd.data = torch.flip(self.weight_fwd.data, [2, 3])
                self.output = inv_conv_4d(input, self.weight_fwd)
                self.output = torch.flip(self.output, [2, 3])

            else :
                # input = torch.flip(input, [3])
                # self.weight_fwd = torch.flip(self.weight_fwd, [2, 3])
                self.output = inv_conv_4d(input, self.weight_fwd)
                # self.output = torch.flip(self.output, [3])
            ldj = 0.0
    ...
    def plot_filters(self, layer_idx, max_s=10):
        ...
        for direction in ['fwd', 'inv']:
            ...
            plt.close('all')
class inv_flow_no_pad(FlowLayer):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
    ...
    def reset_parameters(self):
        ...
        # self.mask = self.get_mask()
        # print('inv_flow, reset_parameters before masking ', self.weight_fwd.data)
        for c_out in range(self.weight_fwd.data.shape[0]):
            # if c_out == self.weight_fwd.data.shape[0] - 1:
            self.weight_fwd.data[c_out, -1, -1, -1] = 1.0
            # self.weight_fwd.data[c_out, c_out+1:, -1, -1] = 0.0
        # print('inv_flow, reset_parameters after masking', self.weight_fwd.data)
