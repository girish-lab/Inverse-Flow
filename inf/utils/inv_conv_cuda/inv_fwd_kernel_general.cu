...
__global__ void inv_conv_fwd_kernel(
    const at::PackedTensorAccessor<scalar_t,4,at::RestrictPtrTraits,size_t> input,
    const at::PackedTensorAccessor<scalar_t,4,at::RestrictPtrTraits,size_t> kernel,
    at::PackedTensorAccessor<scalar_t,4,at::RestrictPtrTraits,size_t> output,
    int c,
    int d,
...
std::vector<at::Tensor> inv_conv_fwd_cuda_inverse(
    at::Tensor input,
    at::Tensor kernel,
    at::Tensor output) {
    ...
    for (int c = 0; c < (int) C / 4; c++) {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "inv_conv_fwd_cuda_inverse", ([&] {
          inv_conv_fwd_kernel<scalar_t><<<blocks, threads>>>(
              input.packed_accessor<scalar_t,4,at::RestrictPtrTraits,size_t>(),
              kernel.packed_accessor<scalar_t,4,at::RestrictPtrTraits,size_t>(),
              output.packed_accessor<scalar_t,4,at::RestrictPtrTraits,size_t>(), 
              c,
              d,
              relevant_threads,
              order_stride
