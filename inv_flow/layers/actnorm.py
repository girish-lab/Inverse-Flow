...
class ActNorm(FlowActivationLayer):
    def __init__(self, n_dims):
        ...
        self.n_dims = n_dims
        self.translation = torch.nn.Parameter(torch.zeros(n_dims)) #, bias=False) ## bias=False
        self.log_scale = torch.nn.Parameter(torch.zeros(n_dims)) #, bias=False) # bias=True
        self.register_buffer('initialized', torch.tensor(0))

    def forward(self, input, context=None):
        reduce_dims = [i for i in range(len(input.size())) if i != 1]
        # print(' Actnorm, fwd input:', input.shape)
