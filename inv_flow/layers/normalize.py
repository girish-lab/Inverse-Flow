...
class Normalization(PreprocessingFlowLayer):
    ...
    def __init__(self, translation, scale, learnable=False):
        ...
        if learnable:
            self.translation = torch.nn.Parameter(torch.Tensor([translation]), bias=False) # bias=False
            self.scale = torch.nn.Parameter(torch.Tensor([scale]), bias=True) # bias=True
        else:
