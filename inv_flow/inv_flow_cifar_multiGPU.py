...
class Inv_FlowStep(nn.Module):
    def __init__(self, size, actnorm=True):
        super().__init__()
        inv_flow_unit = inv_flow(size[0], size[0], (3, 3))
        glow_unit = GlowStep(size, actnorm)
        dict = OrderedDict([
          ('inv_flow_unit', inv_flow_unit),
          ('glow_unit', glow_unit)
        ])
        # self.inv_flow_step = nn.ModuleList({
        #     'inv_flow_unit': inv_flow_unit,
        #     'glow_unit': glow_unit
        # })
        self.inv_flow_step = nn.Sequential(dict)
    def forward(self, x):
        logdet = 0
        # x, layer_logdet = self.inv_flow_unit(x, context=None)
        # logdet += layer_logdet
        # x, layer_logdet = self.glow_unit(x, context=None)
        # logdet += layer_logdet
        for layer in self.inv_flow_step:
            x, layer_logdet = layer(x)#, context=None)
            logdet += layer_logdet
    ...
    def reverse(self, x):
        # print("In Inv_Flow Step Inverse")
        for layer in reversed(self.inv_flow_step):
            # print(layer)
            x = layer.reverse(x)#, context=None)
...
class Inv_FlowLevel(nn.Module):
    def __init__(self, size, block_size=16, actnorm=False):
        ...
        self.inv_low_level = nn.ModuleList([
            squeeze,
            *[Inv_FlowStep(size, actnorm) for _ in range(block_size)],
            split
        ])
    def forward(self, x):
        logdet = 0
        ...
        # x, layer_logdet = self.glow_unit(x, context=None)
        # logdet += layer_logdet
        for layer in self.inv_low_level:
            if not isinstance(layer, SplitPrior):
                x, layer_logdet, z = layer(x)
            else:
    ...
    def reverse(self, x, z=None):
        for layer in reversed(self.inv_low_level):
            # print("In inv_FlowLevel Reverse")
            # print(layer)
            if not isinstance(layer, SplitPrior):
                x = layer.reverse(x)#, context=None)
            else:
...
class Inv_Flow(nn.Module):
    def __init__(self, n_blocks=2, block_size=16, image_size=(1, 28, 28), actnorm=False):
        ...
        self.inv_flow_levels = nn.ModuleList([Inv_FlowLevel((C_in * (2**i), H//(2**i), W//(2**i)), block_size, actnorm) for i in range(n_levels)])
        self.squeeze = Squeeze()
        self.inv_flow_step = nn.Sequential(*[Inv_FlowStep(self.output_size, actnorm) for _ in range(block_size)])
    ...
    def forward(self, x, context=None):
        ...
        for module in self.inv_flow_levels:
            x, z, layer_logdet = module(x)
            logdet += layer_logdet
            zs.append(z)
        # print("After inv_flow_levels", x.shape, torch.cuda.current_device())
        x, layer_logdet = self.squeeze(x)
        logdet += layer_logdet
        # print("Outside Squeeze:", x.shape, torch.cuda.current_device())
        for module in self.inv_flow_step:
    ...
    def reverse(self, n_samples=1, zs=None, z_std=1.0):
        ...
        z = zs[-1]
        for module in reversed(self.inv_flow_step):
            z = module.reverse(z)
        x = self.squeeze.reverse(z)

        for i, m in enumerate(reversed(self.inv_flow_levels)):
...
if __name__ == '__main__':
    ...
    config = {
        'name': f'3L-32K Inv_Flow_CIFAR_{run_name}',
        'eval_epochs': 1,
        'sample_epochs': 1,
        'log_interval': 10,
        'lr': lr,
        ...
        'dataset': 'CIFAR',
        'run_name': f'{run_name}',
        'wandb_project': 'Inv-flow-CIFAR-Matched',
    ...
    model = Inv_Flow(n_blocks=config['num_blocks'],
