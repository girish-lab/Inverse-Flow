...
class Inv_Flow(nn.Module):
    def __init__(self, n_blocks=2, block_size=16, image_size=(1, 28, 28), actnorm=False):
        ...
        self.fastflow_levels = nn.ModuleList([Inv_FlowLevel((C_in * (2**i), H//(2**i), W//(2**i)), block_size, actnorm) for i in range(n_levels)])
        self.squeeze = Squeeze()
        self.fastflow_step = nn.Sequential(*[Inv_FlowStep(self.output_size, actnorm) for _ in range(1)])
...
if __name__ == '__main__':
    ...
    config = {
        'name': f'2L-16K Inv_Flow_MNIST_{run_name}',
        'eval_epochs': 1,
        'sample_epochs': 1,
        'log_interval': 100,
        'lr': lr,
        ...
        'dataset': 'MNIST',
        'run_name': f'{run_name}',
        'wandb_project': 'Inv_Flow-MNIST-WithoutActNorm',
    ...
    # grad clamping for exploding gradients, value is set to 1.0
    # for p in model.parameters():
    #     p.register_hook(lambda grad: torch.clamp(grad, -config['grad_clip'], config['grad_clip']))
    model = model.to('cuda')
