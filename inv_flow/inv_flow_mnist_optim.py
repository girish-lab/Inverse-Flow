...
def create_model(num_blocks=2, block_size=16, activation='Spline', sym_recon_grad=False, 
    ...
    for b in range(num_blocks):
        ...
        for k in range(block_size):
            ...
            layers.append(Coupling(current_size, width=128)) # width=512 (san)
            if not (b == num_blocks - 1 and k == block_size - 1):
...
def main():
    ...
    scheduler = ExponentialLR(optimizer, gamma=0.98, last_epoch=-1, verbose=True) # gamma= 0.997, 0.998, 0.999, 0.9999, 0.99999
